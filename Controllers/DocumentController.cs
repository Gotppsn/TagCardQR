...
public async Task<IActionResult> DownloadDocument(int id, string token = null)
{
    try
    {
        ...
        {
            // For PDFs requested in preview context, return content directly
            bool isPdf = document.FilePath.ToLower().EndsWith(".pdf") || 
                         document.FileType?.ToLower().Contains("pdf") == true;
            bool isPreview = Request.Query.ContainsKey("preview");
            ...
            if (isPdf && isPreview)
            {
                try {
                    ...
                    var response = await client.GetAsync(document.FilePath);
                ...
                {
                    _logger.LogError(ex, $"Error fetching PDF content from URL: {document.FilePath}");
